...
DataGOL is a data platform where teams organize records in workbooks made up of rows and typed columns. This node lets a workflow add rows, update rows, and query rows in a workbook, and lets a workflow start automatically whenever a row is added or updated — so you can sync data between two workbooks, or between DataGOL and any other n8n-connected system, in either direction.
...
|---|---|
| Add | Insert a new row into a workbook. Cell values are mapped through a schema-aware column mapper built from the workbook's real column list. |
| Update | Update an existing row by its row ID. Only the columns you map are changed; other cells are left untouched. |
| Get Many | Query rows from a workbook with an optional raw WHERE clause, multi-column sorting, and a return-all or limit/page-size option. |
...
A polling trigger that starts a workflow when rows change in a workbook:

| Watch For | Description |
|---|---|
| Row Added | Fires once per new row that appears in the workbook. |
| Row Updated | Fires once per row whose selected column changes. |

The trigger needs a **Date Column** to key change-detection off of. Any column in the workbook can be picked, but a DATE-typed audit column (DataGOL's own `created_at`/`updated_at`) works best and is recommended and listed first. It uses that column, plus the workbook's primary key, to detect and de-duplicate changes across polls without emitting the same row twice.
...
1. A DataGOL account with access to the workspace/workbook you want to automate.
...
2. Pick the **Workspace** and **Workbook** from the list (or paste an ID).
3. Use **Cell Values** to map incoming data to the workbook's columns.
...
Add a **DataGOL Trigger** node, choose **Row Added** or **Row Updated**, pick the workbook, and select a Date Column (an audit column such as `created_at`/`updated_at` is recommended, but any column works).

### Avoiding self-triggering loops

A common pattern is: *"When a row changes in Workbook A, update a related row in Workbook A or Workbook B."* If the workflow writes back to the **same** workbook that a Row Updated trigger is watching, that write bumps the row's own `updated_at` column — which the trigger will see on its next poll, potentially re-running the workflow.
